...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
		...
		<div class="container d-flex align-items-center">
			...
			<nav class="nav-menu d-none d-lg-block">
				<ul>
					...
					<li><a href="${pageContext.request.contextPath}/gallery/list">공존소식</a></li>
        			...
        			<c:otherwise>
 	        		<li class="drop-down p-0" id="loginNavLink"><a href="${pageContext.request.contextPath}/checkmember">${member.nickname} <i class="fas fa-user-circle"></i></a>
						<ul>
							<li><a href="${pageContext.request.contextPath}/checkmember">마이페이지</a></li>
							<li><a href="${pageContext.request.contextPath}/logout" class="logoutLink">로그아웃</a></li>
						</ul>
					</li>      				
        			</c:otherwise>
	        		</c:choose>	
	...
	<script>
		...
		$(function() {
			...
			// 로그아웃 시 현재 주소값 같이 보내기
			$(".logoutLink").on("click", function(e) {
				e.preventDefault();
				var url = cp + "/logout?url=" + window.location.href;
				location.href=url;
			})
		})	
	</script>
