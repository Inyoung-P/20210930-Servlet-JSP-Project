			...
			<div class="container">
				...
				<form method="post" class="form-group m-0" enctype="multipart/form-data">
					<div class="portfolio-description pt-0">
						...
						<label for="noticeTitle">제목</label>&nbsp;&nbsp;&nbsp;<span class="text-muted small" id="titleCount"></span>
						<input type="text" name="title" class="form-control mt-2 mb-3" placeholder="제목을 30자 이내로 입력하세요." id="noticeTitle" value="${board.title}" />
						<label for="noticeContent">내용</label>&nbsp;&nbsp;&nbsp;<span class="text-muted small" id="contentCount"></span>
						<textarea name="content" rows="10" class="form-control mb-3" placeholder="내용를 입력하세요." id="noticeContent" >${board.content}</textarea>
						<label>첨부파일</label>
						<div class="fileInput">
					...
					<div class="row">
						<div class="col-lg-12 d-flex justify-content-end">
							<button class="portfolio-button-bottom mr-2" id="btnModifyNotice"><i class="far fa-save"></i> 수정</button>
							<button class="portfolio-button-bottom" id="btnCancleModifyNotice" onclick="cancleModifyNotice()"><i class="fas fa-times"></i> 취소</button>
							<div class="modal fade" id="cancleModifyNoticeModal">
								<div class="modal-dialog">
									<div class="modal-content">
										...
										<div class="modal-footer">
											<button type="button" class="btn btn-info" onclick="btnNoticeDetail()">확인</button>
...
<script>
	...
	$(function() {
		...
		// 글 수정
 		$("#btnModifyNotice").click(function() {
			event.preventDefault();
			var titleLen = $("#noticeTitle").val().trim().length;
			var contentLen = $("#noticeContent").val().trim().length;
			if(!titleLen) {
				$("#noticeTitle").focus().val('').attr('placeholder', '제목에 공백만 입력할 수 없습니다.');
				return;
			}
			else if(!contentLen) {
				$("#noticeContent").focus().val('').attr('placeholder', '내용에 공백만 입력할 수 없습니다.');
				return;
			}
			else {
			    // 첨부파일1 변경
	...
	// 글 수정 취소
	function cancleModifyNotice() {
		event.preventDefault();
		$("#cancleModifyNoticeModal").modal();
	}
	// 글 수정 취소 확인 모달창의 확인 버튼
	function btnNoticeDetail() {
		event.preventDefault();
		location.href= cp + '/notice/detail?bno='+'${board.bno}';	
	}
	// 첨부파일1 삭제
