				...
				<div class="portfolio-description">
					<c:if test="${member.id == board.id && member.emailChecked == 1}">
						<div class="col-lg-12 d-flex justify-content-end">
							<button class="portfolio-button-bottom mr-2 noticeModify" data-bno="${board.bno}"><i class="fas fa-pencil-alt"></i> 수정</button>
							<button class="portfolio-button-bottom noticeDelete" data-bno="${board.bno}"><i class="far fa-trash-alt"></i> 삭제</button>
							<div class="modal fade" id="deleteNoticedModal">
								<div class="modal-dialog">
									<div class="modal-content">
										...
										<div class="modal-footer">
											<button type="button" class="btn btn-info btnDeleteNotice">확인</button>
					...
					<h2 class="d-inline" style="word-break: break-all">${board.title}</h2>
					...
					<div class="portfolio-details-container text-right">
						<c:if test="${not empty board.attachs}">
							<div><i class="far fa-folder"></i> 첨부파일 <span id="attachCount"></span>
						</c:if>
					...
					<div class="portfolio-details-container mt-5 mb-5">
						<div style="word-break: break-all; white-space: pre;" id="noticeContent"><c:out value="${board.content}"/></div>
					</div>
				</div>
				<div class="col-lg-12 d-flex justify-content-end">
					<button class="portfolio-button-bottom" onclick="location.href='${pageContext.request.contextPath}/notice/list?pageNum=${param.pageNum}&amount=${param.amount}'"><i class="far fa-list-alt"></i> 목록</button>
				</div>
	...
	<script>
		...
		$(function() {
			...
			$(document).on("click", ".noticeDelete", function(e) {
				e.preventDefault();
				$("#deleteNoticedModal").modal();
			});
			
			// 글 삭제 모달창에서 확인
			$(document).on("click", ".btnDeleteNotice", function(e) {
				e.preventDefault();
				$("#deleteNoticedModal").modal('hide');
				var url = cp + "/notice/delete?bno=" + bno;
				$.ajax(url, {
					method:"delete",
			...
			$(document).on("click", ".noticeModify", function(e) {
				e.preventDefault();
				var bno = $(this).data("bno");
				location.href = cp + "/notice/modify?bno=" + bno;
			});
